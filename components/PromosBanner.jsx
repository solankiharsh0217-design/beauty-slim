...
export default function PromosBanner() {
  ...
  const loadPromos = async () => {
    setLoading(true)
    try {
      const data = await adminAuth.getActivePromos()
      setPromos(data)
    } catch (e) {
      ...
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
        {visible.map(promo => (
          <div key={promo.id} className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <Link href="/contatti" className="inline-flex items-center gap-1.5 text-primary font-semibold text-xs shrink-0 hover:gap-2.5 transition-all duration-300">
